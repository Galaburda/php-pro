...
use App\Services\SuperVisor\Iterators\SupervisorProcessesIterator;
use Exception;
...
class SuperVisorService
{
    ...
    public function addProcessesConfig(ProcessDTO $dto): void
    {
        try {
            ...
            if ($config->hasSection(self::SECTION_PREFIX . $dto->getName())
                === true
            ) {
                return;
            }
    ...
    public function deleteProcess(string $processName): void
    {
        try {
            $filePath = config('supervisor.configPath');
            $config = (new IniFileLoader($filePath))->load();
            if ($config->hasSection(self::SECTION_PREFIX . $processName)
                === false
            ) {
                return;
            }
    ...
    {
        try {
            $process = $this->supervisor->getAllProcesses();
            return new SupervisorProcessesIterator($process);
        } catch (Exception $exception) {
        }
    }
}
